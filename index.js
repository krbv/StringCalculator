    class CalculatorFromString {
        ...
        unwrap() {
            ...
            match.map(item => {
                ...
                this.curStr = this.curStr.replace(item, `${this.expressionToNumber(exp)}`);
            });
            if (str.includes('(')) {
                this.unwrap();
            }
        ...
        chunkParser(chunk) {
            const arr = chunk.split(this.exp.main);
            [arr[1], arr[3]] = [Number(arr[1]), Number(arr[3])];
            switch (arr[2]) {
                case '/': return arr[1] / arr[3];
                case '+': return arr[1] + arr[3];
                case '-': return arr[1] - arr[3];
                case '*': return arr[1] * arr[3];
            }
        ...
        unwrap() {
            ...
            match.map(item => {
                ...
                this.curStr = this.curStr.replace(item, `${this.expressionToNumber(chunk)}`);
            });
        ...
        prioritySetter() {
            ...
            if (check) {
                check.map(item => {
                    this.curStr = this.curStr.replace(item, `${this.chunkParser(item)}`);
                })
            }
            return this;
        }
